...
def read_processed_meta(processed_dir: Path) -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.read_csv(m, on_bad_lines='skip')
    return df
...
def run_from_config(cfg_path: Path):
    ...
    # 중복 제거 로직 강화
    initial_count = len(meta_all)
    if 'record_key' in meta_all.columns:
        # record_key 기준 중복 제거
        duplicates_mask = meta_all.duplicated(subset=['record_key'], keep='first')
        if duplicates_mask.any():
            duplicate_count = duplicates_mask.sum()
            print(f"[Dedup] Removing {duplicate_count} duplicate records based on record_key")
            # 중복되지 않은 인덱스만 유지
            keep_indices = ~duplicates_mask
            X_all = X_all[keep_indices]
            meta_all = meta_all[keep_indices].reset_index(drop=True)
            print(f"[Dedup] Records reduced: {initial_count} -> {len(meta_all)}")
    
    # gkey 기준 추가 중복 확인 (concat/fuse 모드용)
    if 'gkey' in meta_all.columns and ms_mode != "per_band":
        gkey_duplicates = meta_all.duplicated(subset=['gkey'], keep='first')
        if gkey_duplicates.any():
            gkey_dup_count = gkey_duplicates.sum()
            print(f"[Dedup] Removing {gkey_dup_count} duplicate groups based on gkey")
            keep_gkey_indices = ~gkey_duplicates
            X_all = X_all[keep_gkey_indices]
            meta_all = meta_all[keep_gkey_indices].reset_index(drop=True)
