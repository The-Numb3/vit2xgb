...
def process_dataset(dataset_root: Path, out_dir: Path, overwrite: bool=False, pad: int=4):
    result_root = dataset_root
    if not result_root.is_dir():
        raise SystemExit(f"[ERR] dataset 폴더가 없습니다: {result_root}")
    ...
    for p in result_root.glob("*"):
        ...
        # 1) 폴더명과 동일한 파일명 시도 (140093200156_day7.xlsx)
        cand1 = p / f"{base}.xlsx"
        if cand1.is_file():
            csv_list.append(cand1)
            continue
            
        # 2) day 제거한 파일명 시도 (140093200156.xlsx) 
        slug = base.split("_day")[0]
        cand2 = p / f"{slug}.xlsx"
        if cand2.is_file():
            csv_list.append(cand2)
    ...
    for i, csv_path in enumerate(csv_list, 1):
        ...
        for _, row in df.iterrows():
            ...
            for wl in wls:
                ...
                # 원본 이미지 파일명 패턴 시도 (여러 변형)
                img_path = ""
                
                # 1) 기본 패턴: <이력번호>_s<세션>_<파장>nm
                img_stem1 = f"{slaughter}_{session}_{wl}nm"
                if img_stem1 in img_index:
                    img_path = img_index[img_stem1]
                
                # 2) day가 폴더명에 있는 경우 파일명에도 day 추가
                if not img_path and day:
                    img_stem2 = f"{slaughter}_{session}_{wl}nm_day{day}"
                    if img_stem2 in img_index:
                        img_path = img_index[img_stem2]
                
                # 3) case_name이 _day7 형태인 경우 (_day 추출)
                if not img_path and "_day" in case_name:
                    day_suffix = case_name.split("_day")[1]  # "7" 추출
                    img_stem3 = f"{slaughter}_{session}_{wl}nm_day{day_suffix}"
                    if img_stem3 in img_index:
                        img_path = img_index[img_stem3]
                
                # 4) 마지막으로 rglob으로 패턴 검색
                if not img_path:
                    maybe = list((case_dir / case_name.split("_day")[0]).rglob(f"*{session}*{wl}nm*.png"))
                    if maybe:
                        img_path = str(maybe[0])
                if not img_path or not os.path.isfile(img_path):
                    print(f"[MISS] 이미지 없음: {img_stem1} (csv={csv_path})")
                    continue
